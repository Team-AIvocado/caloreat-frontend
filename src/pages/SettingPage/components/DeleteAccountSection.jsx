...
export const DeleteAccountSection = ({ currentUser }) => {
    ...
    if (!isExpanded) {
        return (
            <div className="bg-[#FFD7D7] p-6 rounded-lg shadow-sm mb-4 border border-red-300 flex justify-between items-center">
                <h3 className="text-lg font-bold text-red-600">회원 탈퇴</h3>
                <button
                    onClick={() => setIsExpanded(true)}
                    className="px-4 py-2 text-white bg-red-500 rounded-md hover:bg-red-600 transition-colors font-medium border-transparent shadow-sm"
                >
                    탈퇴하기
                </button>
            </div>
    ...
    /* 위험 안내 문구 표시 */
    return (
        <div className="bg-white p-6 rounded-lg shadow-sm mb-4 border border-red-100">
